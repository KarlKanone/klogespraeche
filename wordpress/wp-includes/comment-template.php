...
function comments_number( $zero = false, $one = false, $more = false, $deprecated = '' ) {
	...
	if ( $number > 1 )
		$output = str_replace('%', number_format_i18n($number), ( false === $more ) ? __('%') : $more);
	elseif ( $number == 0 )
		$output = ( false === $zero ) ? __('No Comments') : $zero;
	else // must be one
		$output = ( false === $one ) ? __('1') : $one;
	 ...
	 */
	echo apply_filters( 'comments_number', $number );
}
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	echo ' title="' . esc_attr( sprintf( __('Comment on %s'), $title ) ) . '"><span class="glyphicon glyphicon-comment" id="comment"></span><p class="comment">';
